...
Calendar.prototype = {
	...
	Draw : function() {
		...
		Yue = this.Month > 9 ? this.Month : "0" + this.Month;
		this.Days = [];
		while (arr.length > 0) {
			// 每个星期插入一个tr
			...
			for ( var i = 1; i <= 7; i++) {
				...
				if (arr.length > 0) {
					...
					var d = arr.shift();
					cell.innerHTML = "<span id='" + this.Year + Yue + d + "'>"
							+ d + "</span>";
					if (d > 0 && day.length) {
						for ( var ii = 0; ii < day.length; ii++) {
							this.Days[d] = cell;
							// 已签到
							if (this.IsSame(new Date(this.Year, this.Month - 1,
									d), day[ii])) {
								this.onToday(cell);
							}
							// 判断今天是否签到
	...
	SignIn : function() {
		...
		for ( var i = 0; i < tb.rows.length; i++) {
			for ( var j = 0; j < tb.rows[i].cells.length; j++) {
				if (day == tb.rows[i].cells[j].innerText && Year == this.Year
						&& Month == this.Month) {
